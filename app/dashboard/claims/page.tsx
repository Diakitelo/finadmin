import Link from 'next/link';
import TableHeader from '@/components/table-header';
import prisma from '@/lib/prisma';
import { z } from 'zod';
import { revalidatePath } from 'next/cache';
import { formatDate, formatTime } from '@/utils/formatDate';

export const revalidate = 0;

const deleteClaimSchema = z.object({
  id: z.coerce.number(),
});

async function handleDeleteCustomer(formData: FormData) {
  'use server';

  const form = Object.fromEntries(formData);

  const data = deleteClaimSchema.parse(form);

  await prisma.claim.delete({
    where: {
      id: data.id,
    },
  });

  revalidatePath('/dashboard/claims');
}

export default async function Claim() {
  const claims = await prisma.claim.findMany({
    include: {
      operation: {
        include: {
          customer: true,
        },
      },
    },
  });

  return (
    <div className="space-y-5">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-semibold text-black">Réclamations</h2>

        <Link
          href="claims/new"
          className="flex items-center bg-primary rounded-md text-white font-medium gap-1 px-3 py-2.5 text-sm"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-6 h-6"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M12 6v12m6-6H6"
            />
          </svg>
          <span>Ajouter un client</span>
        </Link>
      </div>

      <div className="w-full bg-white rounded-lg p-5">
        <div className="overflow-hidden">
          <table className="min-w-full text-black">
            <TableHeader data={CustomerTableHeader} />
            <tbody>
              {claims.map((claim) => (
                <tr key={claim.id}>
                  <td className="text-left py-3 px-5">
                    {claim.operation.customer.lastName}{' '}
                    {claim.operation.customer.lastName}
                  </td>
                  <td className="text-left py-3 px-5">{claim.operation.id}</td>
                  <td className="text-left py-3 px-5">{claim.amount}</td>
                  <td className="text-left py-3 px-5">
                    {formatDate(claim.createdAt)} à{' '}
                    {formatTime(claim.createdAt)}
                  </td>{' '}
                  <td className="text-left py-3 px-5">{claim.status}</td>
                  <td className="text-left py-3 px-5 flex gap-1.5">
                    <Link
                      href={`claims/${claim.id}/details`}
                      className="text-primary hover:bg-gray-50 p-2 rounded-full"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth={1.5}
                        stroke="currentColor"
                        className="w-5 h-5"
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"
                        />
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"
                        />
                      </svg>
                    </Link>

                    <form action={handleDeleteCustomer}>
                      <button className="text-red-500 hover:bg-gray-50 p-2 rounded-full">
                        <input type="hidden" name="id" value={claim.id} />
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                          viewBox="0 0 24 24"
                          strokeWidth={1.5}
                          stroke="currentColor"
                          className="w-5 h-5"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                          />
                        </svg>
                      </button>
                    </form>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}

const CustomerTableHeader = [
  'Client',
  "Id de l'opération",
  'Montant',
  'Date',
  'Status',
  'Actions',
];
